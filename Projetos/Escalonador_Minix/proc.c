...
double rand_num() {
    next = (next * 1103515245ULL + 12345ULL) % 18446744073709551615ULL;
    unsigned int rand_num = (unsigned int)(next / 65536) % 10001;
	return (double)rand_num / 100;
}
 ...
 */
   ...
   */
  int flag_aux = 0;
  u64_t max_time_in_queue;
  register struct proc *aux_rp;	
  register struct proc *first_rp;

  rdy_head = get_cpulocal_var(run_q_head);
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	rp = rdy_head[q];			/* start at head of queue */
	if (rand_num() > 90){
		// Se o processo for do usuario, devemos escolher com base no tempo de execucao
		if((rp = rdy_head[q]) != NULL && rp->p_priority >= USER_Q){
			max_time_in_queue = 0;
			first_rp = rdy_head[q];
			aux_rp = rdy_head[q];
			for (rp = first_rp; (!((first_rp != rp)^(flag_aux == 0))) && (rp != NULL); rp = rp->p_nextready) {
				flag_aux = 1;
				if(rp->p_accounting.time_in_queue > max_time_in_queue){
					max_time_in_queue = rp->p_accounting.time_in_queue;
					aux_rp = rp;
				}
			}
			rp = aux_rp;
